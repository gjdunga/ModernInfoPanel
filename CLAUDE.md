...
- **Compile-check:** `make references-managed` (one-time) then `make build` — type-checks
  the single `oxide/plugins/ModernInfoPanel.cs` against the real Oxide/Rust/Unity assemblies
  (net48). See `BUILD.md`. The shipped artifact is the raw `.cs`; the build DLL is throwaway.
- **Frameworks:** the plugin targets **Oxide** and is also **Carbon** compatible — it uses
  only the shared Rust/CUI APIs (no Reflection, no framework-specific calls).
